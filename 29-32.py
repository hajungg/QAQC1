...
# 페이지 기본 설정
st.set_page_config(page_title="Chill & NASA Battery Dashboard", layout="wide")

# ✅ 로고 크기를 키우고 위치를 내리기
col1, col2 = st.columns([8, 1])  # 오른쪽에 로고 배치
with col2:
    st.markdown("<br>", unsafe_allow_html=True)  # ✅ 로고를 아래로 이동
    st.image(r"C:\Users\hajun\Desktop\cooding\dash\image (4).png", width=200)  # ✅ 크기 두 배로 키움


# 사이드바 메뉴 추가
menu = st.sidebar.selectbox("메뉴 선택", ["고객용 대시보드", "기업용 대시보드"], index=1)
